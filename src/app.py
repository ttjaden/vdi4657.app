...
button_download = dbc.Button(
    html.Div(id='button_download_content',children=[DashIconify(icon='ic:baseline-download'),html.Div('Parameter Download')]),
    id='button_download',
    outline=True,
    color="primary",
    active=True,
...
content = dbc.Container([
    ...
    dcc.Store(id='parameter_building_type'),                                                            # Gebäudetyp -> df                               
...
def change_solar2_style(n_clicks):
    if (n_clicks%2)==1:
        return {'background-color': '#003da7','color': 'white',}
    else:
...
@app.callback(
    ...
    State('absolut_bat_cost_small', 'value'),
    State('absolut_bat_cost_big', 'value'),
...
def download(n1, last_triggered_building, n_clicks_pv, n_clicks_chp, n_clicks_hp, building, price_electricity,absolut_bat_cost_small, absolut_bat_cost_big, parameter_lang, parameter_use, parameter_location_string, parameter_pv, parameter_chp, parameter_hp):
    ...
    df['absolut_bat_cost_small']=[absolut_bat_cost_small] #TODO
    df['absolut_bat_cost_big']=[absolut_bat_cost_big]
    df['Building_size']=[Area]
...
def calc_bat_results(e_hh,building_name,building_type, heating,region,Area,building_type_data, choosen_hp, chp_max_ratio, choosen_chp, pv_size, chp_active, hp_active, pv_active):
    ...
    if pv_active['color']!='white':
        ...
        type='a'
        year=2015
        c_pv=sim.calc_pv(trj=region-1,year=year,type=type,tilt=tilt,orientation=orientation)
        p_pv=np.array(c_pv)*pv_size

    ## heating system
    if (heating is None) or (region is None) or (Area is None) or(building_type_data is None):
        p_hp = 0
        p_chp = 0
        hp_summary=None
        p_chp = 0
        ...
        if building_type_data.endswith('unsaniert'):
            building=buildings.loc['old',:]
        elif building_type_data.startswith('Bestand'):
            building=buildings.loc['renovated',:]
        elif building_type_data.startswith('Neubau'):
        ...
        if (choosen_hp is None) or hp_active['color']!='white':
            ...
            if choosen_hp.startswith('Sole'):
                ...
                raise PreventUpdate
            P_hp_el , results_summary, t_in, t_out = sim.calc_hp(building,load_dict,group_id)
            p_hp= P_hp_el.values
        ...
        if (choosen_chp is None) or chp_active['color']!='white':
            ...
            results_timeseries, _, P_th_chp, P_el_chp, runtime = sim.calc_chp(building,load_dict,choosen_chp/100,chp_to_peak_ratio=chp_max_ratio/100)
...
def economic_results_graph(batteries,batteries_peak,electricity_price,electricity_price_peak,absolut_bat_cost_small,absolut_bat_cost_big,tab,results_id, LSK, lang):
